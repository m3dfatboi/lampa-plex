(function () {
    ...
    function historyAllItemKey(item) {
        ...
        var title = cardTitle(item) || item.title || item.name || '';
        var season = item && (item.seasonIndex || item.parentIndex || item.season || item.seasonNumber || '');
    ...
    function historyWholeShowLabel(item, rowKind) {
        ...
        if (type == 'season' || sourceType == 'season') {
            season = item.seasonIndex || item.index || item.parentIndex || item.season || item.seasonNumber || '';
            season = parseInt(season, 10) || season;

            return season ? episodeLabel(season, '') : 'Full';
        }
