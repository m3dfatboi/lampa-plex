(function () {
    ...
    function registerPlexRows() {
        ...
        Lampa.ContentRows.add({
            ...
            call: function () {
                ...
                return safeContentRow(function (call) {
                    loadPlexHistory(function (cards) {
                        call(makePlexRow('История', cards));
                    }, function () {
                        call();
                    });
                });
        ...
        Lampa.ContentRows.add({
            ...
            call: function (params) {
                ...
                return safeContentRow(function (call) {
                    loadPlexWatchlistTypeRow('movie', function (cards) {
                        call(makePlexRow(WATCHLIST_TITLE, cards));
                    }, function () {
                        call();
                    });
        ...
        Lampa.ContentRows.add({
            ...
            call: function (params) {
                ...
                return safeContentRow(function (call) {
                    loadPlexWatchlistTypeRow('show', function (cards) {
                        call(makePlexRow(WATCHLIST_TITLE, cards));
                    }, function () {
                        call();
                    });
    ...
    function loadPlexWatchlistTypeRow(wanted, success, fail) {
        request('GET', DISCOVER + WATCHLIST_PATH, {
            ...
            'X-Plex-Container-Start': 0,
            'X-Plex-Container-Size': PLEX_ROW_SIZE * 4
        }, function (data) {
            var items = [];

            collectMetadata(data).forEach(function (item) {
                var type = normalizePlexType(item && item.type);

                if (!isWatchlistRowType(item, wanted)) return;

                if (wanted == 'show' && type == 'season') item = showHistoryItem(item);
                if (item) items.push(item);
            });

            items = items.slice(0, PLEX_ROW_SIZE);
            ...
            hydratePlexItems(items, 'watchlist', success, {
                watchlisted: true
            });
        }, fail);
    }
    ...
    function loadPlexHistory(success, fail) {
        loadProfileWatchHistory(function (items) {
            ...
            if (!history.length) {
                loadPlexHistoryFallback(success, fail);
                return;
            }
