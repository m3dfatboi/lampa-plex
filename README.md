...
- Добавляет `Добавить в Очередь` / `Удалить из Очереди`, `Оценить на Plex` и отметку выбранной серии в меню действий карточки.
...
- Подменяет нативные ряды истории Lampa на Plex: `История`, `Новые эпизоды`, `Вы смотрели`.
- Добавляет ряды `История` и `Очередь` на главную страницу Lampa, а также фильтрованную `Очередь` в разделы фильмов и сериалов.
- Показывает сохранённые Plex-ряды сразу, а `Историю` незаметно сверяет с Plex и обновляет прямо на открытой странице.
- Сопоставляет карточки сначала по TMDB/IMDb id из Plex; поиск по названию используется только при точном совпадении названия и года.
- Убирает с карточек иконки истории, очереди и закладок — как добавленные плагином, так и нативные Lampa.
- Авторизуется в Plex по коду через официальный PIN-flow, без ручного поиска токена.
- Может отмечать фильмы и серии как просмотренные в Plex, когда прогресс Lampa достигает выбранного порога или серия отмечена вручную в списке серий.
...
- Если Lampa держит старый кэш: `https://m3dfatboi.github.io/lampa-plex/plex-watchlist.js?v=0.5.22`
