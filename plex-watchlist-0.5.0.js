(function () {
    ...
    function showHistoryItem(item) {
        ...
        if (type == 'show') return item;

        if (type != 'episode' && !isPlexEpisodeLike(item)) return null;
        ...
        out = {
            ...
            title: title,
            year: year,
            originallyAvailableAt: item.grandparentOriginallyAvailableAt || (year ? year + '-01-01' : ''),
            thumb: item.grandparentThumb || item.parentThumb || item.thumb,
            art: item.grandparentArt || item.parentArt || item.art,
            key: item.grandparentKey || (showKey ? '/library/metadata/' + showKey : item.key),
